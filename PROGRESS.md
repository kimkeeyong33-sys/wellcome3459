...
- 열려 있는 PR:
  - PR #5 https://github.com/kimkeeyong33-sys/wellcome3459/pull/5 — 프로필 완성하기(상호명/성명/이메일) + 사업자등록증 첨부·인증. **Supabase 대시보드에서 비공개 버킷(`business-licenses`)을 직접 만들어야 함 — 아래 "다음에 할 일" 참고**
- 병합 완료 (기본 브랜치에 모두 반영됨): PR #1(견적함 메뉴+Toast), #2(회원가입 카카오 상단배치+업체명), #3(PWA 설치 배너 수정), #4(회원번호+추천 링크 공유)
...
5. **프로필 완성하기 + 사업자등록증 인증** — 마이페이지에 상호명/성명/이메일(전부 선택) 저장 섹션과 사업자등록증 업로드(비공개 Storage 버킷, `members.business_license_path`) 추가. 업로드하면 "인증 대기중" 상태가 되고, 관리자가 서명 URL로 열람 후 "인증 완료 처리"하면 "✓ 인증된 사업자" 배지가 마이페이지/관리자 회원 목록/리드 목록에 표시됨. → PR #5, 리뷰 대기 중.

## 다음에 할 일

- [ ] PR #5 리뷰 후 병합
- [ ] PR #5 병합 전후로 Supabase 대시보드 → Storage에서 **`business-licenses` 버킷을 직접 생성**해야 함 — 반드시 "Public bucket" 체크 해제(비공개 유지). 정책(RLS) 설정은 불필요(서버가 service_role 키로만 접근). 자세한 절차는 PR #5 설명과 `supabase/schema.sql`의 해당 주석 참고
- [ ] 실제 Supabase 프로젝트의 SQL Editor에서 `supabase/schema.sql`의 마이그레이션 블록(`member_no`, `name`/`email`/`business_license_path` 등)을 아직 실행 안 했다면 실행 필요 (이 세션엔 연결된 Supabase 프로젝트가 없어 로컬에서 직접 검증하지 못했음)
