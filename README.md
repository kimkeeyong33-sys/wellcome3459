...
## 하향경매 · 공동구매 (점핑비드 확장 기능)

기존 "덤핑점핑" 알림 흐름에 더해, 점핑비드의 핵심 거래 방식 중 두 가지를 추가했습니다.

### 하향경매 (`/auctions`)

- 관리자가 `/admin`에서 시작가·바닥가·하락폭·하락 주기·수량·진행 시간을 정해 등록합니다.
- 가격은 서버 계산 없이, 클라이언트에서 `starts_at` 기준 경과 시간으로 실시간 계산됩니다
  (`src/lib/auction.ts`의 `computeAuctionPrice`). 바닥가 밑으로는 내려가지 않습니다.
- 회원가입 없이도 "지금 이 가격에 구매하기"를 누르면 전화번호·수량만으로 신청이 접수되고,
  Postgres 함수 `place_auction_order`가 재고를 원자적으로 차감합니다(동시 구매 정합성 보장).
- 실제 결제는 매물과 동일하게 점핑매니저가 연락해서 진행합니다(온라인 결제는 아직 없음).

### 공동구매 (`/groupbuys`)

- 관리자가 수량 구간별 단가(`tiers`, 오름차순), 성사 최소 수량, 마감시간을 정해 등록합니다.
- 참여 수량이 늘어날수록 단가가 자동으로 내려가고(`src/lib/groupbuy.ts`), 목표 수량 미달 시
  마감되면 관리자가 "성사 불발 취소"로 처리합니다(전원 결제 없음).
- 참여도 전화번호·수량만으로 접수되며, Postgres 함수 `join_group_buy`가 참여 수량을 누적합니다.

두 기능 모두 Supabase 미설정 상태(로컬 데모)에서는 `src/lib/mockData.ts`의 목업 데이터로 그대로
동작합니다. 실 서비스로 쓰려면 `supabase/schema.sql`을 다시 실행해서 `auctions`, `group_buys`
등 관련 테이블/함수를 추가하세요(기존에 실행한 적이 있어도 안전하게 재실행 가능합니다).
